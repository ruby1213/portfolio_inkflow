  ...
  useEffect(() => {
    ...
    const splatShader = `
      ...
      void main(){
        ...
        gl_FragColor = vec4(min(base + splat, vec3(1.0)), 1.0);
      }`;
    ...
    const progGradientSubtract = createProgram(
      baseVertex,
    ...
    const COLORS = [
      [0.1, 0.1, 0.11], // sumi black
      [0.08, 0.16, 0.28], // indigo
      [0.55, 0.14, 0.09], // vermilion
      [0.1, 0.22, 0.13], // pine green
    ];
    ...
    const COLORS_DARK = [
      [0.62, 0.6, 0.55], // pale sumi glow
      [0.2, 0.48, 0.92], // luminous indigo
      [0.92, 0.34, 0.24], // luminous vermilion
      [0.3, 0.68, 0.42], // luminous pine
    ];
    ...
    function splat(x, y, dx, dy, color) {
      ...
      gl.uniform1f(
        progSplat.uniforms.aspectRatio,
    ...
    function step(dt) {
      ...
      gl.uniform2f(
        progVorticity.uniforms.texelSize,
        1 / simRes.width,
        1 / simRes.height,
      );
      ...
      gl.uniform2f(
        progDivergence.uniforms.texelSize,
        1 / simRes.width,
        1 / simRes.height,
      );
      ...
      gl.uniform2f(
        progPressure.uniforms.texelSize,
        1 / simRes.width,
        1 / simRes.height,
      );
      ...
      gl.uniform2f(
        progGradientSubtract.uniforms.texelSize,
        1 / simRes.width,
        1 / simRes.height,
      );
      gl.uniform1i(
        progGradientSubtract.uniforms.uPressure,
        pressure.read.attach(0),
      );
      gl.uniform1i(
        progGradientSubtract.uniforms.uVelocity,
      ...
      gl.uniform2f(
        progAdvection.uniforms.texelSize,
        1 / simRes.width,
        1 / simRes.height,
      );
      ...
      gl.uniform1f(
        progAdvection.uniforms.dissipation,
        (1 - params.VELOCITY_DISSIPATION) * 2.2,
      );
      blit(velocity.write);
      velocity.swap();
      ...
      gl.uniform1f(
        progAdvection.uniforms.dissipation,
        (1 - params.DENSITY_DISSIPATION) * 2.2,
      );
      blit(dye.write);
      dye.swap();
    }
    ...
    function normX(clientX) {
      return clientX / canvas.width;
    }
    function normY(clientY) {
      return 1 - clientY / canvas.height;
    }
    ...
    function movePointer(id, clientX, clientY) {
      ...
      if (dist > 1) {
        ...
        splat(
          normX(p.x),
          normY(p.y),
          dx * params.SPLAT_FORCE * 0.02,
          dy * params.SPLAT_FORCE * 0.02,
          p.color,
        );
      }
      idleTimer = Date.now();
    ...
    const onPointerMove = (e) => {
      if (pointers[e.pointerId] !== undefined)
        movePointer(e.pointerId, e.clientX, e.clientY);
      else startPointer(e.pointerId, e.clientX, e.clientY);
      idleTimer = Date.now();
    };
    ...
    function applyWash() {
      ...
      gl.uniform1f(progClear.uniforms.value, 0.9 - Math.min(t * 0.02, 0.05));
      blit(dye.write);
      dye.swap();
      for (let i = 0; i < 2; i++) {
        const x = Math.random(),
